...
/**********************************
 * CS Dashboard - Unassigned (IFRAME Frontend)
 * By: Andres Garcia 
 **********************************/
...
$backendUrl    = rtrim($API_HOST, '/') . '/pstools/script/cs-dashboard-unassigned-sql';
...
<|div class="kpi-row">
  <|div class="kpi-card card-pending active" data-status="Pending">
    <|div class="kpi-left"><|div class="kpi-label">Unassigned<|/div><|div class="kpi-value" id="kpi-count-Pending">-<|/div><|/div>
    <|div class="kpi-right"><|i class="fas fa-hand-paper"><|/i><|/div>
  <|/div>
  ...
  <|div class="row">
    ...
    <|div class="col-md-2 mb-2">
      ...
      <|select id="case_status" class="form-control">
        ...
        <|option value="Pending" selected>Unassigned<|/option>
...
<|div id="csLoader">
  <|div class="cs-loader-box">
    ...
    <|div class="cs-loader-text">Please wait while the Unassigned dashboard is being prepared.<|/div>
  <|/div>
<|/div>
HTML;
...
<script>
(function(){
  var BACKEND="__BACKEND__", CURRENT_USER=__CURRENT_USER__, TAB="unassigned";
  ...
  // Shared payload used by table, KPI, and export requests.
  ...
  // Loads the process dropdown from the init response.
  ...
  // Background init call for process list and KPIs.
  ...
  // Refreshes KPI cards with the current filter values.
  ...
  function scheduleBackgroundLoads(){
    setTimeout(function(){ loadInitOnly(); },80);
  }
    ...
    try{
      ...
      dtTable=$("#casesTable").DataTable({
        ...
        ajax:{
          ...
          contentType:"application/json",
          data:function(d){
            return JSON.stringify(Object.assign({},buildBasePayload(),{
              draw:d.draw,start:d.start,length:d.length,order:d.order,columns:d.columns
            }));
          },
          dataSrc:function(json){
            json=unwrap(json);
            if(!json){ hideLoader(); return []; }
          ...
          $.ajax({
            ...
            success:function(resp){
              ...
              try{
                ...
                XLSX.writeFile(wb,"Unassigned.xlsx");
