 ...
 * Uses PSTools slug process-export-batch (Process_Export_Batch.php). Markup uses <| … </| for ProcessMaker.
 */
...
$slug = preg_replace('/[^a-z0-9\-_]/', '', (string) ($data['pstools_slug_process_export'] ?? 'process-export-batch'));
if ($slug === '') {
    $slug = 'process-export-batch';
}
...
<style>
  ...
  #inpPstoolsUrl { word-break: break-all; font-size: 11px; }
  #lblPstoolsUrlEcho { word-break: break-all; font-family: ui-monospace, monospace; }
</style>
CSS;
  ...
  <|div class="pex-card">
    ...
    <|div class="form-row align-items-end">
      <|div class="col-md-4 mb-2">
        ...
        <|/div>
        <|div class="pex-hint mt-1">Host for REST calls comes from <|strong>API base<|/strong> above. Optional prod fields help when <|code>api_base<|/code> is empty (Forte host resolution). Auth is always <|code>bearer_token<|/code> only.<|/div>
      <|/div>
      <|div class="col-md-4 mb-2">
      ...
      <|/div>
      <|div class="col-md-4 mb-2">
        <|label class="small font-weight-bold text-secondary mb-1">PSTools batch URL (display)<|/label>
        <|input type="text" id="inpPstoolsUrl" class="form-control form-control-sm bg-light" readonly="readonly" value="__URL_BATCH_ESC__" title="" />
        <|div class="pex-hint mt-1">POSTs use this path built from <|strong>API base<|/strong> + <|code>/pstools/script/<|/code> + slug — not a truncated field value. Slug must match your registered PSTools script (default <|code>process-export-batch<|/code>).<|/div>
        <|small id="lblPstoolsUrlEcho" class="text-muted d-block mt-1"><|/small>
      <|/div>
    <|/div>
    <|div class="form-row">
      <|div class="col-md-6 mb-2">
      ...
      <|/div>
      <|div class="col-md-6 mb-2">
        <|label class="small font-weight-bold text-secondary mb-1" for="inpProdToken">Prod token (optional, unused for API)<|/label>
        <|input type="password" id="inpProdToken" class="form-control form-control-sm" autocomplete="off" />
        <|div class="pex-hint mt-1">REST calls use only <|strong>Bearer token<|/strong> above. This field is not sent as <|code>bearer_token<|/code>.<|/div>
      <|/div>
    <|/div>
    <|div class="form-row">
...
(function(){
  ...
  function batchSlug(){
    var rd = CONFIG.requestData || {};
    var s = rd.pstools_slug_process_export || "process-export-batch";
    s = String(s).replace(/[^a-z0-9\-_]/g, "");
    return s || "process-export-batch";
  }

  /** Always derive from current API base + slug so POST never uses a stale/truncated readonly field. */
  function getPstoolsBatchUrl(){
    var b = apiBase();
    if (b) {
      return b + "/pstools/script/" + batchSlug();
    }
    var fallback = (CONFIG.pstoolsUrl && String(CONFIG.pstoolsUrl).trim()) ? String(CONFIG.pstoolsUrl).trim() : "";
    return fallback;
  }

  function describeXhrFail(xhr, textStatus, errorThrown){
    if (xhr != null && typeof xhr === "string") {
      return xhr;
    }
    var lines = [];
    if (xhr && xhr.status) {
    ...
    if (textStatus && textStatus !== "error") lines.push("status: " + textStatus);
    if (errorThrown) lines.push(String(errorThrown));
    ...
    if (body) {
      ...
      lines.push("(empty response body — check URL, slug, CORS, and that PSTools returns JSON)");
    }
    return lines.join("\n");
  }

  function postBatch(body, ajaxOpts){
    var url = getPstoolsBatchUrl();
    if (!url) return $.Deferred().reject("no pstools url — set API base URL");
    return $.ajax($.extend({
      url: url,
      type: "POST",
  ...
  function updatePstoolsUrl(){
    var full = getPstoolsBatchUrl();
    $("#inpPstoolsUrl").val(full);
    $("#inpPstoolsUrl").attr("title", full);
    $("#lblPstoolsUrlEcho").text(full ? ("Effective POST URL: " + full) : "Set API base to build PSTools URL.");
  }

  function loadCounts(){
    clearErr();
    if (!getPstoolsBatchUrl()) {
      showErr("Set API base URL (same as Postman …/api/1.0) so PSTools can be called.");
      $("#outPre").text("Cannot build PSTools URL: API base is empty and the screen has no API_HOST fallback.");
      return;
    }
    postBatch(mergePayload({ action: "counts" })).done(function(r){
      ...
      showErr("Counts request failed" + (xhr && xhr.status ? " (HTTP " + xhr.status + ")" : "") + ". See Last response for details.");
      $("#outPre").text(describeXhrFail(xhr, textStatus, errorThrown));
    });
  }
  ...
  function initTable(){
    ...
    tbl = $("#tblProcesses").DataTable({
      ...
      ajax: function (dtParams, callback) {
        var batchUrl = getPstoolsBatchUrl();
        if (!batchUrl) {
          showErr("Set API base URL — cannot call PSTools.");
          $("#outPre").text("getPstoolsBatchUrl() is empty. Enter the same API base as Postman (…/api/1.0).");
        ...
        $.ajax({
          ...
          showErr("List request failed" + (xhr && xhr.status ? " (HTTP " + xhr.status + ")" : "") + ".");
          $("#outPre").text(describeXhrFail(xhr, textStatus, errorThrown));
          callback({ draw: dtParams.draw || 1, recordsTotal: 0, recordsFiltered: 0, data: [] });
        });
      },
  ...
  $("#btnManifest").on("click", function(){
    ...
    }).fail(function(xhr, ts, te){
      showErr("Manifest request failed.");
      $("#outPre").text(describeXhrFail(xhr, ts, te));
    });
  });
  ...
  $("#btnExport").on("click", function(){
    ...
    postBatch(mergePayload({
      ...
      showErr("Process export request failed.");
      $("#outPre").text(describeXhrFail(xhr, ts, te));
    });
  });
  ...
  $("#inpMode, input[name=inpTarget], #inpProdHost").on("change blur", function(){
