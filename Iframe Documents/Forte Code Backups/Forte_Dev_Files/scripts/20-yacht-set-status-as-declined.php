...
if ($data['YQP_STATUS'] == "BOUND") {
    $dataProducing = array();
    $dataProducing['YQP_STATUS'] = "DECLINED";
...
$dataEndCase = array();
//Set value as Declined
$dataEndCase['YQP_STATUS'] = "DECLINED";
...
if ($data['_request']['process_id'] != getenv('FORTE_ID_YACHT')) {
    ...
    $dataOriginalSave['FORTE_OD_CANCEL'] = "DECLINED";  
    ...
    //Delete Data Collection Premium
    $dataCollectionPremium = array();
    $dataCollectionPremium['FORTE_EP_REQUEST'] = $data['_request']['id'];
    $searchRequestUrlDelete = getenv('API_HOST') . '/collections/' . getenv('FORTE_ENDORSEMENT_PREMIUM_ID') . '/records?pmql=(data.FORTE_EP_REQUEST="' . $dataCollectionPremium['FORTE_EP_REQUEST'] . '")';
    $searchRequestDelete = callGetCurl($searchRequestUrlDelete, "GET", "");
    if (count($searchRequestDelete["data"]) != 0) {
        $deleteRequestUrl = getenv('API_HOST') . '/collections/' . getenv('FORTE_ENDORSEMENT_PREMIUM_ID') . '/records/' . $searchRequestDelete["data"][0]["id"];
        $deleteRequest = callGetCurl($deleteRequestUrl, "DELETE", "");
    }
}
//Return all Values
return $dataEndCase;