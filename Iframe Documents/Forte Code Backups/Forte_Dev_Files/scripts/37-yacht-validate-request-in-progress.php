...
if (strlen(trim($clientName)) > 0) {
    ...
    if (strlen(trim($vesselName)) > 0) {
        ...
        $sqlSearchRequest = "
                ...
                JSON_OBJECT(
                    ...
                    'BOUND_STATUS',
                        CASE
                            ...
                            AND DATE_SUB(
                                    STR_TO_DATE(PR.data->>'$.YQP_PERIOD_TO', '%Y-%m-%d'),
                                    INTERVAL 3 MONTH
                                )
            ...
            AND PR.id != " . $requestId . "
            AND LOWER(TRIM(JSON_UNQUOTE(JSON_EXTRACT(PR.data, '$.YQP_CLIENT_NAME')))) COLLATE utf8mb4_general_ci LIKE LOWER(TRIM('%{$clientName}%'))
            AND LOWER(TRIM(JSON_UNQUOTE(JSON_EXTRACT(PR.data, '$.YQP_INTEREST_ASSURED')))) COLLATE utf8mb4_general_ci LIKE LOWER(TRIM('%{$vesselName}%'))
            AND PRT.created_at = (
                SELECT MAX(created_at)
                FROM process_request_tokens
                WHERE process_request_id = PR.id
            )
            AND PR.data->>'$.YQP_STATUS' != 'QUOTED, NOT TAKEN'
