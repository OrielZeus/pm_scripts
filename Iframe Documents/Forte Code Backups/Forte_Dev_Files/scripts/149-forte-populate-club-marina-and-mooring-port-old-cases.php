<?php
/*****************************************************  
 * Populate club marina and mooring port of old cases
 *
 * by Cinthia Romero
 ****************************************************/
/* 
...
for ($i = 0; $i <= count($responseSearchYacht['data']); $i++) {
    if ($responseSearchYacht['data'][$i]['id'] == 1240) {
        ...
        if (count($searchRequest["data"]) > 0) {
            ...
            //Saving mooring port and club marina to report
            $mooringPortReport = empty($yachtRequestData['YQP_MOORING_PORT']) ? " " : $yachtRequestData['YQP_MOORING_PORT'];
            if (empty($yachtRequestData['YQP_LOCATION_MOORING_PORT']) != true && $yachtRequestData['YQP_LOCATION_MOORING_PORT'] == "Other") {
                $mooringPortReport = empty($yachtRequestData['YQP_SPECIFY_PORT']) ? " " : $yachtRequestData['YQP_SPECIFY_PORT'];
            }
            $dataSendGestion['YQP_MOORING_PORT_REPORT'] = $mooringPortReport;
            $dataSendGestion['YQP_CLUB_MARINA'] = empty($yachtRequestData['YQP_CLUB_MARINA']) ? " " : $yachtRequestData['YQP_CLUB_MARINA'];
            //
