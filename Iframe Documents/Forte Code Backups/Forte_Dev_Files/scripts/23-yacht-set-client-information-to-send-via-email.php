...
if ($approveSrUnderwriter == "YES") {
    $dataClientInfo['YQP_DOWNLOAD_REVIEW_SLIP_DOCUMENT_URL'] = $_SERVER["HOST_URL"] . '/request/' . $data['_request']['id'] . '/files/' . $data['YQP_UPLOAD_QUOTE_SLIP']; 
    ...
    $idFileQuote = $data['YQP_UPLOAD_QUOTE_SLIP'];
} else {
...
$urlEmailCollection = getenv('API_HOST') . '/collections/' . getenv('FORTE_EMAIL_COLLECTION') . '/records' . '?include=data&pmql=' . urlencode('(data.FORTE_EMAIL_PROCESS = "' . $data['_request']['process_id'] . '" and data.FORTE_EMAIL_TYPE = "QUOTE" and data.FORTE_EMAIL_LANGUAJE = "' . $data['YQP_LANGUAGE'] . '")');
...
if (count($responseEmailCollection['data']) > 0) {
    ...
    if ($data['YQP_QUOTE_DOCUMENTS'] == null) {
        $data['YQP_QUOTE_DOCUMENTS'] = array();
    }
    for ($i = 0; $i < count($data['YQP_QUOTE_DOCUMENTS']); $i++) {
        if( $data['YQP_QUOTE_DOCUMENTS'][$i]['YQP_QUOTE_UPLOAD_DOCUMENT'] != null) {
            //Get Document Name
            $urlDocument = getenv('API_HOST') . '/files/' . $data['YQP_QUOTE_DOCUMENTS'][$i]['YQP_QUOTE_UPLOAD_DOCUMENT'];
            $responseDocument = callGetCurl($urlDocument, "GET", "");
            //Get Document Content
            $fileAttach = callGetCurlFiles($data['YQP_QUOTE_DOCUMENTS'][$i]['YQP_QUOTE_UPLOAD_DOCUMENT'], $responseDocument["file_name"]);
            //Attach document
            $message->attach(Swift_Attachment::fromPath($fileAttach)->setFilename($responseDocument["file_name"]));
            //Add document name to the list of additional documents
            $nameAttachments .= "<li>" . $data['YQP_QUOTE_DOCUMENTS'][$i]['YQP_QUOTE_ATTACHMENT_NAME'] . "</li>";
        }
    }
    ...
    //Add CC from Screen
    if ($data['YQP_SLIP_EMAIL_CC'] == null) {
        $data['YQP_SLIP_EMAIL_CC'] = array();
    }
    for ($z = 0; $z < count($data['YQP_SLIP_EMAIL_CC']); $z++) {
        if ($data['YQP_SLIP_EMAIL_CC'][$z]['YQP_EMAIL_CC'] != null || $data['YQP_SLIP_EMAIL_CC'][$z]['YQP_EMAIL_CC'] != '') {
            $message->addCc($data['YQP_SLIP_EMAIL_CC'][$z]['YQP_EMAIL_CC']);
        }        
    }       
    // Set to Replay
    if ($reply == true) {
