...
for (let i = 0; i < this.END_CONFIRMATION_DOCUMENTS.length; i++) {
    //Check that the Name is not null or empty
    if (this.END_CONFIRMATION_DOCUMENTS[i]['END_CONFIRMATION_ATTACHMENT_NAME'] === null || 
        this.END_CONFIRMATION_DOCUMENTS[i]['END_CONFIRMATION_ATTACHMENT_NAME'] == "" || 
        this.END_CONFIRMATION_DOCUMENTS[i]['END_CONFIRMATION_ATTACHMENT_NAME'] == undefined) {
        if (this.END_CONFIRMATION_DOCUMENTS[i]['END_CONFIRMATION_UPLOAD_DOCUMENT'] === null) {
            //Valid if length is 1
            if (this.END_CONFIRMATION_DOCUMENTS.length == 1) {
                submitVariables['YQP_MESSAGE_ATTACH_NULL_SHOW'] = 'NO';
            } else {
