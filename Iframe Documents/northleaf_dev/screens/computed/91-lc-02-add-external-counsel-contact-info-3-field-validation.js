...
if (!this.PE_CONTACT_INFORMATION_NOT_APPLICABLE) {
	if (!valueIsValid(this.PE_LEGAL_COUNSEL_COMPANY_NAME)) {
		fieldsValidation.isValidCounsel = false;
	}
	if (!valueIsValid(this.PE_LEGAL_COUNSEL_CONTACT_NAME)) {
		fieldsValidation.isValidCounsel = false;
	}
	if (Array.isArray(this.PE_LEGAL_COUNSEL_EMAIL_TABLE) && this.PE_LEGAL_COUNSEL_EMAIL_TABLE.length <= 0) {
		fieldsValidation.isValidCounsel = false;
		fieldsValidation.messageCounsel = "At least one row is required";
	} else {
		let i = 0;
		let counselEmailOK = "YES";
		this.PE_LEGAL_COUNSEL_EMAIL_TABLE.forEach(row => {
			i++;
			if (!valueIsValid(row.PE_LEGAL_COUNSEL_EMAIL)) {				
				fieldsValidation.isValidCounsel = false;
				counselEmailOK = "NO";
			}
		});
