...
$enabledRows  = transformData($enabledRows,true);
$disabledRows = getDisabledRows($apiUrl, $requestId, $teamID);
$disabledRows = transformData($disabledRows);
$viewMode     = 'false';
...
if($teamID == 'PE'){
	$mainText = 'Private Equity';
	$gridTitle = 'Invoice '.$mainText.' Line Items';
}
if($teamID == 'INFRA'){
...
$departmentList    = [];
$officeList        = [];
$flagAllocation    = (isset($data['Allocated_'.$teamID]) AND $data['Allocated_'.$teamID] != '' ) ? true : false;

if($data['Allocated_'.$teamID]){
	$query = 'SELECT data->>"$.NL_ACCOUNT_SYSTEM_ID_ACTG" AS ID, 
				...
				ORDER BY LABEL';
	$accountList = callApiUrlGuzzle($apiUrl, "POST", encodeSql($query));
	...
	$query = 'SELECT ID.data->>"$.NL_COMPANY_SYSTEM_ID_ACTG" AS ID, 
				...
				ORDER BY LABEL';
	$corpProjList = callApiUrlGuzzle($apiUrl, "POST", encodeSql($query));
	...
	$query  = "SELECT data->>'$.NL_ACTIVITY_SYSTEM_ID_ACTG' AS ID, 
				...
				AND ACTIVITY.data->>'$.ACTIVITY_TYPE' IN ('Recoverable','Both') 
				ORDER BY LABEL ASC";
	$activityRecList = callApiUrlGuzzle($apiUrl, "POST", encodeSql($query));

	$query  = "SELECT data->>'$.NL_ACTIVITY_SYSTEM_ID_ACTG' AS ID, 
				data->>'$.ACTIVITY_LABEL' AS LABEL 
				FROM collection_" . getCollectionId('IN_EXPENSE_ACTIVITY', $apiUrl) . " AS ACTIVITY 
				WHERE ACTIVITY.data->>'$.ACTIVITY_STATUS' = 'Active'
				AND ACTIVITY.data->>'$.ACTIVITY_TYPE' IN ('Non-Recoverable','Both') 
				ORDER BY LABEL ASC";
	$activityNoRecList = callApiUrlGuzzle($apiUrl, "POST", encodeSql($query));
...
$theadHtml = '<thead class="thead-light">
				<tr class="trBgWhite">
					...
					<th style="width:200px"></th>
					<th style="width:200px"></th>
					<th colspan="4" style="width:200px"></th>
				</tr>
				<tr class="head-table">
					__TD--TD__
					<th>#</th>
...
$theadHtmlT = str_replace('__TH--TH__', '<th class="pm-bg-primary pm-text-white text-center">{{ IN_INVOICE_CURRENCY }}</th><th class="pm-bg-primary pm-text-white text-center">{{ formatNumber(IN_INVOICE_PRE_TAX) }}</th><th class="pm-bg-primary pm-text-white text-center">{{ formatNumber(IN_INVOICE_TAX_TOTAL) }}</th><th class="pm-bg-primary pm-text-white text-center">{{ formatNumber(IN_INVOICE_TOTAL) }}</th>', $theadHtml);									
$theadHtmlT = str_replace('__TH_ALLOCATION_TH__', '<th>% of Total Invoice Amount</th>', $theadHtmlT);
...
$html .=				'<tbody>
				...
				<div class="table-responsive" id="iframe-table-container" style="max-height: 800px;">
					...
					<table class="table table-bordered table-striped w-auto mb-4">';
$theadHtmlT = str_replace('__TH--TH__', '<th class="pm-bg-primary pm-text-white text-center">{{ IN_INVOICE_CURRENCY }}</th><th class="pm-bg-primary pm-text-white text-center">{{ formatNumber(IN_TOTAL_PRE_TAX) }}</th><th class="pm-bg-primary pm-text-white text-center">{{ formatNumber(IN_TOTAL_TAX) }}</th><th class="pm-bg-primary pm-text-white text-center">{{ formatNumber(IN_SUM_TOTAL) }}</th>', $theadHtml);
$theadHtmlT = str_replace('__TH_ALLOCATION_TH__', '<th>% of Total '.$mainText.' Invoice Amount</th>', $theadHtmlT);
$html .= str_replace('__TD--TD__', '<th v-if="!isDiscrepancyMode"></th><th v-if="!isDiscrepancyMode"></th>', $theadHtmlT);
					...
					$html .= '<tbody>
							...
							>
								...
								<td><!--{{ item.IN_EXPENSE_TEAM_ROW_INDEX }} - -->{{item.IN_EXPENSE_ROW_NUMBER}}</td>
								<td style="min-width:300px">';
								if($data['Allocated_'.$teamID]){
									$html .='<|textarea 
									...
									$html .= '{{item.IN_EXPENSE_DESCRIPTION}}';
								}
								$html .= '</td>
								<td style="min-width:200px">
									<!-- NR R Dropdown -->
								...
								<td style="min-width:300px">
									<!-- Team Routing -->
									<p v-if="item.IN_EXPENSE_TEAM_ROUTING && item.IN_EXPENSE_TEAM_ROUTING.LABEL">{{item.IN_EXPENSE_TEAM_ROUTING.LABEL}}</p>
								</td>
								<td style="min-width:300px">';
								if($data['Allocated_'.$teamID]){
									$html .='
									...
									$html .= '<p v-if="item.IN_EXPENSE_ACCOUNT && item.IN_EXPENSE_ACCOUNT.LABEL">{{item.IN_EXPENSE_ACCOUNT.LABEL}}</p>';
								}
								$html .= '</td>
								<td style="text-align: right; min-width:150px;">
									{{item.IN_EXPENSE_PRETAX_AMOUNT}}
								</td>
								<td style=" text-align: right; min-width:150px;">
									{{item.IN_EXPENSE_HST }}
								</td>
								<td style=" text-align: right; min-width:150px;">
									{{item.IN_EXPENSE_TOTAL }}
								</td>
								...
								<td style="min-width:300px">';
									if($data['Allocated_'.$teamID]){
										...
										$html .= '<p v-if="item.IN_EXPENSE_CORP_PROJ && item.IN_EXPENSE_CORP_PROJ.LABEL">{{item.IN_EXPENSE_CORP_PROJ.LABEL}}</p>';
									}
								$html .= '</td>
								<td style="min-width:300px">
									<p v-if="item.IN_EXPENSE_PROJECT_DEAL && item.IN_EXPENSE_PROJECT_DEAL.LABEL">{{item.IN_EXPENSE_PROJECT_DEAL.LABEL}}</p>
								</td>
								<td style="min-width:300px">
									<!-- Fund Manager Dropdown -->
									<p v-if="item.IN_EXPENSE_FUND_MANAGER && item.IN_EXPENSE_FUND_MANAGER.LABEL">{{item.IN_EXPENSE_FUND_MANAGER.LABEL}}</p>
								</td>
								<td style="min-width:300px">
									<!-- Mandates Dropdown -->
									<multiselect 
										...
										:class="{\'is-invalid-field\': !item.IN_EXPENSE_MANDATE}"
										@input="updateMandate(index, $event)"
									>
									</multiselect>
									<p v-if="!item.IN_EXPENSE_MANDATE" class="text-danger small">
								...
								if($data['Allocated_'.$teamID]){
									...
									$html .= '<p v-if="item.IN_EXPENSE_ACTIVITY && item.IN_EXPENSE_ACTIVITY.LABEL">{{item.IN_EXPENSE_ACTIVITY.LABEL}}</p>';
								}
								$html .= '</td>
								<td style="min-width:300px">
									<!-- Corp Entity Dropdown -->
									<p v-if="item.IN_EXPENSE_CORP_ENTITY && item.IN_EXPENSE_CORP_ENTITY.LABEL">{{item.IN_EXPENSE_CORP_ENTITY.LABEL}}</p>
								</td>
								<td style="min-width:300px">';
								if($data['Allocated_'.$teamID]){
									...
									$html .= '<p v-if="item.IN_EXPENSE_DEPARTMENT && item.IN_EXPENSE_DEPARTMENT.LABEL">{{item.IN_EXPENSE_DEPARTMENT.LABEL}}</p>';
								}
								$html .= '</td>
								<td style="min-width:300px">';
								if($data['Allocated_'.$teamID]) {
									...
									$html .= '<p v-if="item.IN_EXPENSE_OFFICE && item.IN_EXPENSE_OFFICE.LABEL">{{item.IN_EXPENSE_OFFICE.LABEL}}</p>';
								}
								$html .= '</td>
							</tr>
						</tbody>
						<tfoot class="tfoot-light">
							<tr>
								...
								<!--td :class="{matching:true, equal: IN_TOTAL_PERCENTAGE_TOTAL== \'100.00\' , different: \'100.00\' !=IN_INVOICE_TOTAL_PERCENTAGE_INIT_SUB_INFRA}"
									style="text-align: right;padding-right: 1.3em;">
									{{ formatPercentageNumber(IN_TOTAL_PERCENTAGE_TOTAL) }}
								</td-->
								<td></td>
								<td  :class="{matching:true, equal: formatNumber(SUM_PER)== \'100.00\' , different: \'100.00\' != formatNumber(SUM_PER)}"
									style="text-align: right;padding-right: 1.3em;">
									{{ formatPercentageNumber(SUM_PER)}}
								</td>
							...
							<tr v-if="DISCREPANCY_IN_TOTAL_PRE_TAX != 0 || DISCREPANCY_IN_TOTAL_TAX != 0 || DISCREPANCY_IN_SUM_TOTAL != 0 ">
								...
								<td class="matching" style="text-align: right;padding-right: 1.3em;">
									{{ formatNumber(DISCREPANCY_IN_TOTAL_PRE_TAX) }}
								</td>
								<td class="matching" style="text-align: right;padding-right: 1.3em;">
									{{ formatNumber(DISCREPANCY_IN_TOTAL_TAX) }}
								</td>
								<td class="matching" style="text-align: right;padding-right: 1.3em;">
									{{ formatNumber(DISCREPANCY_IN_SUM_TOTAL) }}
								</td>
								<!--td class="matching" style="text-align: right;padding-right: 1.3em;">
								...
								<td></td>
								<td class="matching" style="text-align: right;padding-right: 1.3em;">
									{{ formatPercentageNumber(100 - SUM_PER)}}
								</td>
								<td colspan="8"></td>
							</tr>
						</tfoot>
			...
			<div class="row" style="width: 99%;">
				<div style="width: 25%;">
					<div style="margin-bottom:50px;">
						...
						<|button class="btn btn-success" @click="fixDiscrepancy" v-if="showDiscrepancyButton"><span class="fa fa-wrench"></span> Fix Discrepancy</|button>
				...
				<div style="width: 70%;">
					...
					<div class="alert alert-danger" role="alert" v-if="!isGridValid || gridError.error || fixDiscrepancyError || DISCREPANCY_IN_TOTAL_PRE_TAX != 0 || DISCREPANCY_IN_TOTAL_TAX != 0 || DISCREPANCY_IN_SUM_TOTAL != 0">
						<ul>
							...
							<li v-if="DISCREPANCY_IN_TOTAL_PRE_TAX != 0">The Total Pre Tax Amount does not match the Total ' . $mainText . ' Pre Tax Amount.</li>
							<li v-if="DISCREPANCY_IN_TOTAL_TAX != 0">The Total Tax Amount does not match the Total ' . $mainText . ' Tax Amount.</li>
							<li v-if="DISCREPANCY_IN_SUM_TOTAL != 0">The Total Amount does not match the Total ' . $mainText . ' Total Amount.</li>
...
$html .= "
	...
	// As a plugin
		new Vue({
			...
			data: {
				FLAG_ALLOCATION : '" . $flagAllocation . "',
				...
				summaryTotal: " . json_encode($summaryTotal) . ",
				deletedRows: [],
			...
			methods: {
				...
				addRow() {
					...
					this.items.push(
						{
							...
							IN_EXPENSE_TEAM_ROUTING: {ID: 'INFRA', LABEL: 'Infrastructure'},
					...
					this.items[this.items.length-1]['IN_EXPENSE_TEAM_ROUTING'] = {ID: 'INFRA', LABEL: 'Infrastructure'};
					this.onChangeTeamRouting(this.items.length-1, {ID: 'INFRA', LABEL: 'Infrastructure'});
					this.calculateFinalTotals();
					this.areAllValid() ;
				},
				deleteRow(index) {
				...
				splitRow(index) {
					...
					clonedItem.IN_EXPENSE_PERCENTAGE = '0.00%';
				...
				calculateFinalTotals() {
					...
					if(Array.isArray(this.items)){
						...
						this.items.forEach(item => {
							...
							sumPercentage      += this.cleanNumber(item.IN_EXPENSE_PERCENTAGE);
							if(item.IN_EXPENSE_ROW_NUMBER.includes('.')){								
								rowId    = item.IN_EXPENSE_ROW_NUMBER.split('.')[0];
								rowSubId = item.IN_EXPENSE_ROW_NUMBER.split('.')[1];
								totalRows[rowId] = (totalRows[rowId] == undefined) ? 0 : totalRows[rowId];
								totalRows[rowId] += this.cleanNumber(item.IN_EXPENSE_PERCENTAGE_TOTAL);
								percentPerRow[rowId] = (percentPerRow[rowId] == undefined) ? 0 : percentPerRow[rowId];
								percentPerRow[rowId] += this.cleanNumber(item.IN_EXPENSE_PERCENTAGE_TOTAL);
							}
							else{
								rowId = item.IN_EXPENSE_ROW_NUMBER;
								percentPerRow[item.IN_EXPENSE_ROW_NUMBER] = (percentPerRow[item.IN_EXPENSE_ROW_NUMBER] == undefined) ? 0 : percentPerRow[item.IN_EXPENSE_ROW_NUMBER];
								percentPerRow[item.IN_EXPENSE_ROW_NUMBER] += this.cleanNumber(item.IN_EXPENSE_PERCENTAGE_TOTAL);
								if(this.cleanNumber(item.IN_EXPENSE_PERCENTAGE_TOTAL) != 100){
									invalidRows.push(item.IN_EXPENSE_ROW_NUMBER * 1);
								}
							}
					...
					this.DISCREPANCY_IN_SUM_PERCENT = this.formatNumber(100 - this.cleanNumber(this.SUM_PER));
					if( this.DISCREPANCY_IN_TOTAL_PRE_TAX *1 == 0 && this.DISCREPANCY_IN_TOTAL_TAX *1 == 0 && this.DISCREPANCY_IN_SUM_TOTAL *1 == 0){
					...
					let validRows = this.INVALID_ROWS;
				...
				onBlurTotal(index, field) {
					if(!this.isDiscrepancyMode) {
						...
						let perTotal  = (total * 100) / this.cleanNumber(this.IN_TOTAL_TOTAL_INIT_SUB_INFRA);
						item['IN_EXPENSE_PERCENTAGE_TOTAL'] = isNaN(perTotal) ? '0.00%' : this.formatPercentageNumber(perTotal);
						let expTotal  = (this.cleanNumber(this.IN_TOTAL_PRE_TAX_AMOUNT_INIT_SUB_INFRA) * perTotal)/ 100;
				...
				onBlurPercentageTotal(index, field) {
					if(!this.isDiscrepancyMode) {
						...
						let indexRow = (item['IN_EXPENSE_ROW_NUMBER'].includes('.')) ? item['IN_EXPENSE_ROW_NUMBER'].split('.')[0] : item['IN_EXPENSE_ROW_NUMBER'];
						let totalInG = (this.TOTALS_PER_ROW[indexRow]['IN_EXPENSE_TOTAL'] == undefined) ? 0 : this.TOTALS_PER_ROW[indexRow]['IN_EXPENSE_TOTAL'];
						let totalRow = totalInG * totalPer / 100;
						...
						let partialPer = totalRow * 100 / this.summaryTotal.IN_EXPENSE_TOTAL;

						item['IN_EXPENSE_PERCENTAGE'] = isNaN(partialPer) ? '0.00%' : this.formatPercentageNumber(partialPer);
				...
				onBlurPreTaxAmount(index, field) {
					if(!this.isDiscrepancyMode) {
						...
						let perTotal  = (prevTotal * 100) / this.cleanNumber(this.IN_TOTAL_PRE_TAX_AMOUNT_INIT_SUB_INFRA);
						item['IN_EXPENSE_PERCENTAGE_TOTAL'] = isNaN(perTotal) ? '0.00%' : this.formatPercentageNumber(perTotal);
						let expTotal  = (this.cleanNumber(this.IN_TOTAL_TOTAL_INIT_SUB_INFRA) * perTotal)/ 100;
				...
				fixDiscrepancy(){
					...
					let fixedPreTax = this.cleanNumber(this.items[itemsSize-1]['IN_EXPENSE_PRETAX_AMOUNT'])*1 + this.DISCREPANCY_IN_TOTAL_PRE_TAX * 1;
					this.items[itemsSize-1]['IN_EXPENSE_PRETAX_AMOUNT'] = this.formatNumber(fixedPreTax);
					let fixedHst = this.cleanNumber(this.items[itemsSize-1]['IN_EXPENSE_HST'])*1 + this.DISCREPANCY_IN_TOTAL_TAX * 1;
					this.items[itemsSize-1]['IN_EXPENSE_HST'] = this.formatNumber(fixedHst);
					let fixedTotal = this.cleanNumber(this.items[itemsSize-1]['IN_EXPENSE_TOTAL'])*1 + this.DISCREPANCY_IN_SUM_TOTAL * 1;
					this.items[itemsSize-1]['IN_EXPENSE_TOTAL'] = this.formatNumber(fixedTotal);
					let idIndex = this.items[itemsSize-1]['IN_EXPENSE_TEAM_ROW_INDEX'];
					let totalPerIndex = this.TOTALS_PER_ROW[idIndex].IN_EXPENSE_TOTAL;
					let fixPerc = fixedTotal * 100 / totalPerIndex;
					this.items[itemsSize-1]['IN_EXPENSE_PERCENTAGE_TOTAL'] = this.formatPercentageNumber(fixPerc);
					if(this.FLAG_ALLOCATION){
						//this.onBlurPercentageTotal(itemsSize-1,'IN_EXPENSE_PERCENTAGE_TOTAL');
					}
			...
			mounted() {
				this.validateGridA = false;
				if(Array.isArray(this.items)) {
					let totalsPerRow = [];
					let indexRow     = '';
					this.items.forEach((item, index) => {
						indexRow = (item.IN_EXPENSE_ROW_NUMBER.includes('.')) ? item.IN_EXPENSE_ROW_NUMBER.split('.')[0] : item.IN_EXPENSE_ROW_NUMBER;
						if(totalsPerRow[indexRow] == undefined){
							totalsPerRow[indexRow] = [];
							totalsPerRow[indexRow]['IN_EXPENSE_PRETAX_AMOUNT'] = 0;
							totalsPerRow[indexRow]['IN_EXPENSE_HST'] = 0;
							totalsPerRow[indexRow]['IN_EXPENSE_TOTAL'] = 0;
							totalsPerRow[indexRow]['IN_EXPENSE_PERCENTAGE_TOTAL'] = 0;
						}
						totalsPerRow[indexRow]['IN_EXPENSE_PRETAX_AMOUNT'] += item.IN_EXPENSE_PRETAX_AMOUNT * 1;
						totalsPerRow[indexRow]['IN_EXPENSE_HST'] += item.IN_EXPENSE_HST * 1;
						totalsPerRow[indexRow]['IN_EXPENSE_TOTAL'] += item.IN_EXPENSE_TOTAL * 1;
						totalsPerRow[indexRow]['IN_EXPENSE_PERCENTAGE_TOTAL'] += item.IN_EXPENSE_PERCENTAGE_TOTAL * 1;
						...
						//let perRow = (this.cleanNumber(item.IN_EXPENSE_TOTAL) * 100) / this.cleanNumber(this.IN_TOTAL_TOTAL_INIT_SUB_INFRA)
						//item.IN_EXPENSE_PERCENTAGE_TOTAL = isNaN(perRow) ? '0.00%' : this.formatPercentageNumber(perRow);
						/*if(!item.IN_EXPENSE_ROW_NUMBER.includes('.')){
							item.IN_EXPENSE_PERCENTAGE_TOTAL = this.formatPercentageNumber(100);
						}*/
						this.sumPer += this.cleanNumber(this.IN_EXPENSE_PERCENTAGE);
						item.IN_EXPENSE_PERCENTAGE = this.formatPercentageNumber(item.IN_EXPENSE_PERCENTAGE);

						let partialPer = this.cleanNumber(item.IN_EXPENSE_TOTAL) * 100 / this.summaryTotal.IN_EXPENSE_TOTAL;
						item.IN_EXPENSE_PERCENTAGE = isNaN(partialPer) ? '0.00%' : this.formatPercentageNumber(partialPer);
						
						//this.evaluateRow(index);
					});
					this.calculateFinalTotals();
					totalsPerRow.forEach((row, index) => {
						if(row.IN_EXPENSE_PERCENTAGE_TOTAL != 100){
							row.IN_EXPENSE_TOTAL = row.IN_EXPENSE_TOTAL * 100 / row.IN_EXPENSE_PERCENTAGE_TOTAL;
						}
					});
			...
			computed: {
				...
				isValidCustomeGrid() {
					...
					allElementsValid = (
						...
						this.cleanNumber(this.DISCREPANCY_IN_TOTAL_PRE_TAX) == 0 &&
						this.cleanNumber(this.DISCREPANCY_IN_TOTAL_TAX) == 0 &&
						this.cleanNumber(this.DISCREPANCY_IN_SUM_TOTAL) == 0 
						//&& this.isValidAmounts &&
					...
					/*if(
						...
						this.cleanNumber(this.DISCREPANCY_IN_TOTAL_TAX) == 0 &&
						this.cleanNumber(this.DISCREPANCY_IN_SUM_TOTAL) == 0 
						//&& this.cleanNumber(this.DISCREPANCY_IN_SUM_PERCENT) == 0 
					){
...
function getMandateList($collectionId, $apiUrl, $assetLabel) {
	...
	//$query .= empty($assetLabel) ? "" : "AND MANDATE.data->>'$.MANDATE_ASSETCLASS' = '" . $assetLabel . "' "; // display all mandates for all teams
...
function getEnabledRows($apiUrl, $requestId, $task){
	...
	$query .= "order by ";
	$query .= "cast(substring_index(IN_EXPENSE_ROW_NUMBER,'.',1) as unsigned), ";
	$query .= "cast(substring_index(substring_index(IN_EXPENSE_ROW_NUMBER,'.',2),'.',-1) as unsigned) ";

	//$query .= "order by IN_EXPENSE_TEAM_ROW_INDEX + 0";
	//$query .= "cast(substring_index(IN_EXPENSE_ROW_NUMBER,'.',1) as unsigned), ";
	//$query .= "cast(substring_index(substring_index(IN_EXPENSE_ROW_NUMBER,'.',2),'.',-1) as unsigned) ";
	//$query .= " ORDER BY IN_EXPENSE_ROW_NUMBER + 0 ASC";
	$response = callApiUrlGuzzle($apiUrl, "POST", encodeSql($query));
	return $response;
}
/*
 ...
 * by Jhon Chacolla
 */
function transformData($expenseList,$addSubIdex = false) {
	$newExpense = [];
	$sub = 1;
	$main = 1;
	foreach($expenseList as $key => $expense){
		if($addSubIdex){
			$init = $expense['IN_EXPENSE_TEAM_ROW_INDEX'];
			if(isset($expenseList[$key*1+1]) AND $expenseList[$key*1+1]['IN_EXPENSE_TEAM_ROW_INDEX'] == $init){
				$expense['IN_EXPENSE_ROW_NUMBER'] = $main.'.'.$sub;
				$expense['IN_EXPENSE_TEAM_ROW_INDEX'] = $main;
				$sub++; 
			}
			else{
				$expense['IN_EXPENSE_ROW_NUMBER'] = $main.'.'.$sub;
