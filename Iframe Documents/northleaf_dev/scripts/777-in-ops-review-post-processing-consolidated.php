...
$userRole = $teamID . " Ops Approver";
$approval = $data["IN_".$teamID."H_ACTION"] == 'Approved' ? 'Approved' : 'Rejected' ;
$comments = empty($data['IN_'.$teamID.'H_Comments']) ? "" : $data['IN_'.$teamID.'H_Comments'];

//Save comments into collection
$dataRecord = [
    "SUBMIT_02_".$teamID => null,
...
$dataCommentTeam = [
    ...
    'IN_COMMENT_APPROVAL' => $approval,
    'IN_COMMENT_SAVED' => $comments,
    'IN_COMMENT_DATE' => substr($currentDate, 0, 10)
];

$allDataComments = $data["IN_COMMENT_LOG_" . $teamID];
$allDataComments[] = $dataCommentTeam;



$dataReturn = [   
    "CloseThread".$teamID => $approval,
    "IN_COMMENT_LOG" => $commentLogData,
    "Allocated_".$teamID => false,
    "readyScreen" => null,
    "SUBMIT_".$teamID."_H" => null,
    "IN_COMMENT_LOG_" . $teamID => $allDataComments
];

//Assign task titles
if ($teamID == "INFRA") {
    $dataReturn["INFRA_TITLE"]  = "Infrastructure Analyst Review";
    $dataReturn["IN_INFRAH_Comments"] = null;
}

if ($teamID == "PE") {
    $dataReturn["PE_TITLE"] = "Private Equity Analyst Review";
    $dataReturn["IN_PEH_Comments"] = null;
}

if ($teamID == "PC") {
    $dataReturn["PC_TITLE"] = "Private Credit Analyst Review";
    $dataReturn["IN_PCH_Comments"] = null;
}
