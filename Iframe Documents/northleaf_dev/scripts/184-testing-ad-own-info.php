...
class AzureHandler
{
    ...
    public function testConnection()
    {
        try {
            ...
            // Para cada usuario, intenta obtener su manager
            foreach ($users->value as $user) {
                try {
                    $managerResponse = $this->apiClient->get("users/{$user->id}/manager");
                    $manager = json_decode($managerResponse->getBody()->getContents());
                    $user->manager = $manager;
                } catch (BadResponseException $e) {
                    // Si el manager no está disponible, maneja el error y continúa
