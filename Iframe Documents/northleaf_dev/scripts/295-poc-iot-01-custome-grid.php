...
$inTotalPreTaxAmountInitSubInfra = stringToFloat($data['IN_TOTAL_PRE_TAX_AMOUNT_INIT_SUB_INFRA']);
$inTotalHstInitSubInfra = stringToFloat($data['IN_TOTAL_HST_INIT_SUB_INFRA']);
$inTotalTotalInitSubInfra = stringToFloat($data['IN_TOTAL_TOTAL_INIT_SUB_INFRA']);
$inTotalPercentageTotalInitSubInfra = stringToFloat($data['IN_TOTAL_PERCENTAGE_TOTAL_INIT_SUB_INFRA']);
$inInvoicePreTaxPercentageInitSubInfra  = stringToFloat($data['IN_INVOICE_PRE_TAX_PERCENTAGE_INIT_SUB_INFRA']);
$inInvoiceTaxTotalPercentageInitSubInfra  = stringToFloat($data['IN_INVOICE_TAX_TOTAL_PERCENTAGE_INIT_SUB_INFRA']);
$inInvoiceTotalPercentageInitSubInfra  = stringToFloat($data['IN_INVOICE_TOTAL_PERCENTAGE_INIT_SUB_INFRA']);
...
$inTotalPreTaxAmountInitSubInfra = empty($summaryTotal["IN_EXPENSE_PRETAX_AMOUNT"]) ? 0 : stringToFloat($summaryTotal["IN_EXPENSE_PRETAX_AMOUNT"]);
$inTotalHstInitSubInfra          = empty($summaryTotal["IN_EXPENSE_HST"]) ? 0 : stringToFloat($summaryTotal["IN_EXPENSE_HST"]);
$inTotalTotalInitSubInfra        = empty($summaryTotal["IN_EXPENSE_TOTAL"]) ? 0 : stringToFloat($summaryTotal["IN_EXPENSE_TOTAL"]);
...
$html .=				'<tbody>
				...
				<!-- <h2>Team Table</h2> -->
				<h4 class="pm-text-primary">Invoice Infrastructure</h4>
...
$tempHtml = str_replace('__TH_PER_TH__', '<th>% of Total Infrastructure Invoice Amount</th>', $tempHtml);
$html .= str_replace('__TH--TH__', '<th class="pm-bg-primary pm-text-white text-center">{{ formatNumber(IN_TOTAL_PRE_TAX_AMOUNT_INIT_SUB_INFRA) }}</th><th class="pm-bg-primary pm-text-white text-center">{{ formatNumber(IN_TOTAL_HST_INIT_SUB_INFRA) }}</th><th class="pm-bg-primary pm-text-white text-center">{{ formatNumber(IN_TOTAL_TOTAL_INIT_SUB_INFRA) }}</th>', $tempHtml);
$html .=				'<tbody>
						...
						<tfoot class="tfoot-light">
							<tr>
								...
								<td class="matching" 
									v-bind:class="cleanNumber(IN_TOTAL_PRE_TAX_AMOUNT_INIT_SUB_INFRA) == cleanNumber(IN_TOTAL_PRE_TAX_AMOUNT) ? \'equal\' : \'different\'"
									style="text-align: right;padding-right: 1.3em;">
									{{ formatNumber(IN_TOTAL_PRE_TAX_AMOUNT) }}
								</td>
								<td class="matching" 
									v-bind:class="cleanNumber(IN_TOTAL_HST_INIT_SUB_INFRA) == cleanNumber(IN_TOTAL_HST) ? \'equal\' : \'different\'"
									style="text-align: right;padding-right: 1.3em;">
									{{ formatNumber(IN_TOTAL_HST) }}
								</td>
								<td class="matching" 
									v-bind:class="cleanNumber(IN_TOTAL_TOTAL_INIT_SUB_INFRA) == cleanNumber(IN_TOTAL_TOTAL) ? \'equal\' : \'different\'"
									style="text-align: right;padding-right: 1.3em;">
	...
	// As a plugin
		new Vue({
			...
			data: {
				...
				totalPercentage: " . $inInvoiceTotalPercentageInitSubInfra . ",
				//Final Values
			...
			methods: {
				...
				evaluateRow(index) {
					...
					debugger;
					if(item.IN_EXPENSE_VALIDATION.IN_EXPENSE_DEPARTMENT == undefined){
						item.IN_EXPENSE_VALIDATION.IN_EXPENSE_DEPARTMENT = {};
					}
					if(item.IN_EXPENSE_VALIDATION.IN_EXPENSE_OFFICE == undefined){
						item.IN_EXPENSE_VALIDATION.IN_EXPENSE_OFFICE = {};
					}
				...
				onBlurPercentageTotal(index, field) {
					if(!this.isDiscrepancyMode) {
						...
						let pretax  = (totalPer * this.cleanNumber(this.IN_TOTAL_PRE_TAX_AMOUNT_INIT_SUB_INFRA)) / 100;
						item['IN_EXPENSE_PRETAX_AMOUNT'] = isNaN(pretax) ? '0.00' : this.formatNumber(pretax);
						let total  = (totalPer * this.cleanNumber(this.IN_TOTAL_TOTAL_INIT_SUB_INFRA)) / 100;
						...
						// edit to review percentage
						//let prevExpensePercentageTotal = (prevTotal * this.cleanNumber(this.IN_INVOICE_TOTAL_PERCENTAGE_INIT_SUB_INFRA) / this.cleanNumber(this.IN_TOTAL_TOTAL_INIT_SUB_INFRA));
						...
						// Calc IN_EXPENSE_TOTAL = ((IN_EXPENSE_PRETAX_AMOUNT  *  IN_INVOICE_TOTAL_PERCENTAGE_INIT_SUB_INFRA)  /  IN_TOTAL_PRE_TAX_AMOUNT_INIT_SUB_INFRA)
						//let preTaxAmountOld = this.cleanNumber(item['IN_EXPENSE_PRETAX_AMOUNT']);
						let invoicePreTaxPercentage = this.cleanNumber(this.IN_TOTAL_PRE_TAX_AMOUNT_INIT_SUB_INFRA);
						let total = ((preTaxAmount * invoiceTotalPercentage)/ invoicePreTaxPercentage);
						//item['IN_EXPENSE_TOTAL'] = isNaN(total) ? '0.00' : this.formatNumber(total);


						// Calc the posible value of percentage_total
						// Temp cacl Calc IN_EXPENSE_PERCENTAGE_TOTAL = ((IN_EXPENSE_TOTAL  *  IN_INVOICE_TOTAL_PERCENTAGE_INIT_SUB_INFRA)  /  IN_TOTAL_TOTAL_INIT_SUB_INFRA)
						let tempExpensePercentageTotal = (total * invoiceTotalPercentage / this.cleanNumber(this.IN_TOTAL_TOTAL_INIT_SUB_INFRA));
