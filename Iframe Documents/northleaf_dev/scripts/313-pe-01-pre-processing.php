...
$dataReturn = [
   // Clean vars
   'saveButtonFake_subPe' => null,
   'fakeSaveCloseButton_subPe' => null,
   'submitButtonFake_subPe' => null,
   'readyScreen' => null,
   'validateForm_subPe' => null,
   'validation_subPe' => null,
   'saveForm_subPe' => null,
   'saveFormClose_subPe' => null,
   'isCustomeGridValid_subPe' => null
];

// insert data into EXPENSE_TABLE from $data['IN_DATA_GRID_SUB_PE']
if (isset($data['IN_DATA_GRID_SUB_PE'])) {
   $requestId = $data['_request']['id'];
   $query     = "DELETE FROM EXPENSE_TABLE WHERE IN_EXPENSE_CASE_ID = '" . $requestId . "' AND IN_EXPENSE_TEAM_ROUTING_ID = 'PE'";
   $response  = callApiUrlGuzzle($apiUrl, "POST", encodeSql($query));

   $dataGrid    = html_entity_decode($data['IN_DATA_GRID_SUB_PE']);
...
$task = "PE01";
$emailType = "";
if ($data["IN_PE_ASSIGNED"] == "YES") {
   $emailType = "USER";
} else if ($data["IN_PE_ASSIGNED"] == "NO") {
   $emailType = "GROUP";
}
if ($data["IN_SAVE_SUBMIT_SUB_PE"] != "SAVE" && $data["IN_SAVE_SUBMIT_SUB_PE"] != "SAVE_AND_CLOSE") {
   sendInvoiceNotification($data, $task, $emailType, $api);
}

$dataReturn["IN_SAVE_SUBMIT_SUB_PE"] = null;
...
if (!isset($data['IN_COMMENT_LOG_PE']) OR empty($data['IN_COMMENT_LOG_PE'])) {
   $dataReturn['IN_COMMENT_LOG_PE'] = $newCommentLog;
}else{*/
   if (!empty($data['IN_COMMENT_LOG'])) {
      $newDataComments = [];
      foreach ($data['IN_COMMENT_LOG'] as $comment) {
         if ($comment['IN_COMMENT_ROLE'] != 'PC Ops Reviewer' AND $comment['IN_COMMENT_ROLE'] != 'PC Ops Approver' AND $comment['IN_COMMENT_ROLE'] != 'INFRA Ops Reviewer' AND $comment['IN_COMMENT_ROLE'] != 'INFRA Ops Approver') {
            $comment['IN_COMMENT_DATE'] = substr($comment['IN_COMMENT_DATE'], 0, 10);
            $newDataComments[] = $comment; 
         }
      }
      $dataReturn['IN_COMMENT_LOG_PE'] = $newDataComments;
   }
//}
